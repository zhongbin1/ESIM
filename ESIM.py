...
class ESIM(object):
    def __init__(self, max_len_left, max_len_right, vocab_size,
        ...
        with tf.name_scope("composition_layer"):
            cell_fw_2 = tf.nn.rnn_cell.LSTMCell(rnn_size, state_is_tuple=True, name="cell_fw_2")
            cell_fw_2 = tf.nn.rnn_cell.DropoutWrapper(cell_fw_2, output_keep_prob=self.dropout_keep_prob)
            cell_bw_2 = tf.nn.rnn_cell.LSTMCell(rnn_size, state_is_tuple=True, name="cell_bw_2")
            cell_bw_2 = tf.nn.rnn_cell.DropoutWrapper(cell_bw_2, output_keep_prob=self.dropout_keep_prob)

            (output_fw_left, output_bw_left), states_left    = tf.nn.bidirectional_dynamic_rnn(cell_fw_2,
                                                                                                cell_bw_2,
                                                                                                self.F_left,
                                                                                                dtype=tf.float32,
                                                                                                sequence_length=self.length_left)

            (output_fw_right, output_bw_right), states_right = tf.nn.bidirectional_dynamic_rnn(cell_fw_2,
                                                                                               cell_bw_2,
                                                                                               self.F_right,
                                                                                               dtype=tf.float32,
                                                                                               sequence_length=self.length_right)
        ...
        with tf.name_scope("mlp_layer"):
            ...
            self.full_out = tf.nn.dropout(full_out, keep_prob=self.dropout_keep_prob)
